...
public class Permutations {
    ...
    public void extendPermutation(Set<String> remainingItems, Stack<String> permutationSoFar, List<List<String>> allPermutations){
        //UI.println("hello"); // testing
        
        if(!remainingItems.isEmpty()){
            for(String s : remainingItems){
                Stack<String>copyPermutations = (Stack<String>)permutationSoFar.clone(); // copy perm
                Set<String>copyRemaining = new HashSet<String>(Set.copyOf(remainingItems)); // copy remain
                
                copyPermutations.push(s); //push s remaining items
                copyRemaining.remove(s); //remove s remaining items
                
                extendPermutation(copyRemaining,copyPermutations,allPermutations);
                
            }
        } else {
            if(permutationSoFar.size() < 10000){
                List<String>list = new ArrayList<String>(); //new list to store
                
                while(!permutationSoFar.isEmpty()){
                    list.add(permutationSoFar.pop());
